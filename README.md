...
| `history.handoff.{enabled,maxRetries,baseDelayMs,skillFile}` | Compact bằng `handoff` AI (skill `apps/bot/skills/handoff.md`) khi vượt `bot.maxTokenHistory`. AI response → `[HIDDEN_HANDOFF]` injection làm first hidden message. AI viết đầy đủ (không cắt ngắn) để giữ đủ context cho AI kế tiếp. | `true` / `3` / `1500` / unset (built-in `apps/bot/skills/handoff.md`) |
